...
CREATE TABLE Game(game_id INTEGER PRIMARY KEY, secretword VARCHAR, guess_num INTEGER);

DROP TABLE IF EXISTS In_Progress;
CREATE TABLE In_Progress(game_id INTEGER,FOREIGN KEY(game_id) REFERENCES Game(game_id));

DROP TABLE IF EXISTS Completed;
CREATE TABLE Completed(game_id INTEGER, guess_num INTEGER, outcome VARCHAR, FOREIGN KEY(game_id) REFERENCES Game(game_id)); 
